...
const App = () => {
  ...
  const [pauseBtnText, setPauseBtnText] = useState("继续");
  ...
  const closeBtnClickHandler = useCallback(() => {
    const isStart = isStartBtnDisabled;
    const pauseText = pauseBtnText;
    ...
    if (ws) {
      try {
        ...
        setStartBtnDisabled(isStart);
        setPauseBtnText(pauseText);
  ...
  }, [addNewEvent, isCloseBtnDisabled, isStartBtnDisabled, key, pauseBtnText, renderTime, ws]);
    ...
    <div style={containerStyle}>
      <div style={{ height: "100%", margin: "1px", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ margin: "1px" }}>
          <button onClick={startBtnClickHandler} disabled={isStartBtnDisabled}>开始</button>
          <button onClick={pauseBtnClickHandler}>{pauseBtnText}</button>
          <button onClick={closeBtnClickHandler} disabled={isCloseBtnDisabled}>终止</button>
        ...
        <div style={{ height: "15%", margin: "1px",  }}>
          <h4>日志</h4>
            <ul>
              {logData.map((item) => (
                item.error ? 
