  ...
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/") {
      const template = `
        ...
        <button id="pause">继续</button>
        <button id="close" disabled>终止</button>
        <button id="clear" disabled>清空日志</button>
        <h4>日志</h4>
        <ul id="events"></ul>
        <script>
          let ws = null;
          ...
          async function collectWS(command) {
            ...
            ws.addEventListener("open", () => {
              ...
              addNewEvent(">>> 连接远程websocket成功，准备send");
              ...
              if (ws) {
                try {
                  ...
                  //waitReconnect("reconnect", 20000);
                }
              } else {
                //console.log(command + "失败");  //测试
                addNewEvent(">>> " + command + "失败");
                startBtn.disabled = false;
                closeBtn.disabled = true;
              }
            })
            ...
            ws.addEventListener("message", ({ data }) => {
              ...
              if (data === "stop") {
                ...
                addNewEvent(">>> 远程websocket已停止完毕");
                ws.close();
                ws = null;
              } else if (data === "pause") {
                //console.log("远程websocket已暂停完毕");  //测试
                addNewEvent(">>> 远程websocket已暂停完毕");
              } else if (data === "over") {
                over = true;
                //console.log("全部chat采集完毕");  //测试
                addNewEvent(">>>全部chat采集完毕");
              } else {
                addNewEvent(data);
              }
            })
          ...
          async function waitReconnect(command, time) {
            setTimeout(function() {
              //console.log("重新连接远程websocket");  //测试
              //addNewEvent(">>> 重新连接远程websocket");
              ws = null;
              startBtn.disabled = true;
              if (pauseBtn.innerText === "继续") {
          ...
          pauseBtn.addEventListener("click", () => {
            //console.log(pauseBtn.innerText);  //测试
            if (pauseBtn.innerText === "暂停") {
              startBtn.disabled = false;
              pauseBtn.innerText = "继续";
              closeBtn.disabled = false;
        ...
        websocket.addEventListener("message", async ({ data }) => {
          async function open() {
            ...
            try {
              ...
              websocket.send("login出错 : " + e);
              await scheduler.wait(30000);
              await open();
            }
            //console.log("连接服务器成功");
            //websocket.send("连接服务器成功");
          ...
          async function getConfig() {
            ...
            try {
              ...
              if (configResult) {
                ...
                websocket.send("没有预设config");
              }
            } catch (e) {
              //console.log("getConfig出错 : " + e);
              websocket.send("getConfig出错 : " + e);
          ...
          async function updateConfig() {
            ...
            try {
              ...
              if (chatInfo.success === true) {
                ...
                websocket.send("更新config数据成功");
              } else {
                //console.log("更新config数据失败");
                websocket.send("更新config数据失败");
              }
            } catch (e) {
              //console.log("updateConfig出错 : " + e);
              websocket.send("updateConfig出错 : " + e);
              await scheduler.wait(10000);
              await updateConfig();
            }
          }
          ...
          async function getMessage() {
            try {
              ...
              websocket.send("(" + currentStep + ")查询消息出错 : " + e);
              await scheduler.wait(10000);
              await getMessage();
            }
          }
          ...
          async function selectMediaIndex() {
            ...
            try {
              ...
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : selectMediaIndex出错 : " + e);
              await scheduler.wait(10000);
          ...
          async function selectMedia() {
            ...
            try {
              ...
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : selectMedia出错 : " + e);
              await scheduler.wait(10000);
              await selectMedia();
            }
          }
            ...
            if (stop === 0) {
              try {
                ...
                if (hashResult.index === 1) {
                  ...
                  websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 查询首个hash出错 : " + e);
                  await scheduler.wait(5000);
                } else {
                  //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 查询hash出错");
                  websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 查询hash出错" + e);
                  await scheduler.wait(10000);
                  await getHash();
                }
              }
          ...
          async function updateChat() {
            ...
            try {
              ...
              if (chatInfo.success === true) {
                ...
                websocket.send("(" + currentStep + ")更新chat数据成功");
              } else {
                //console.log("(" + currentStep + ")更新chat数据失败");
                websocket.send("(" + currentStep + ")更新chat数据失败");
              }
            } catch (e) {
              //console.log("(" + currentStep + ")updateChat出错 : " + e);
              websocket.send("(" + currentStep + ")updateChat出错 : " + e);
              await scheduler.wait(10000);
              await updateChat();
            }
          }
            ...
            if (hashResult && hashResult.category === 2 && hashResult.hash && hashResult.hash.length && hashResult.hash.length > 0) {
              ...
              try {
                ...
                if (cacheInfo.success === true) {
                  ...
                  websocket.send("(" + currentStep + ")插入cache("+ hashResult.length + " | " + hashResult.index + ")数据成功");
                } else {
                  //console.log("(" + currentStep + ")插入cache("+ hashResult.length + " | " + hashResult.index + ")数据失败");
                  websocket.send("(" + currentStep + ")插入cache("+ hashResult.length + " | " + hashResult.index + ")数据失败");
                }
              } catch (e) {
                ...
                websocket.send("(" + currentStep + ")insertCache("+ hashResult.length + " | " + hashResult.index + ")出错 : " + e);
                await scheduler.wait(10000);
                await insertCache();
              }
            } else {
              //console.log("(" + currentStep + ")cache("+ hashResult.length + " | " + hashResult.index + ")数据错误");
              websocket.send("(" + currentStep + ")cache("+ hashResult.length + " | " + hashResult.index + ")数据错误");
            }
          }
          ...
          //       websocket.send("(" + currentStep + ")更新cache数据成功");
          //     } else {
          //       //console.log("(" + currentStep + ")更新cache数据失败");
          //       websocket.send("(" + currentStep + ")更新cache数据失败");
          //     }
          //   } catch (e) {
          //     //console.log("(" + currentStep + ")updateCache出错 : " + e);
          //     websocket.send("(" + currentStep + ")updateCache出错 : " + e);
          //     await scheduler.wait(10000);
          //     await updateCache();
          //   }
          // }
          ...
          //     websocket.send("(" + currentStep + ")selectCache出错 : " + e);
          //     await scheduler.wait(10000);
          //     await selectCache();
          //   }
          // }
          ...
          async function nextHash() {
            if (stop === 0) {
              if (Date.now() - timeStart < 300000 || (hashResult.index > 0 && hashResult.length - hashResult.index < 50)) {
                if (apiCount < 900) {
                  if (hashResult.offset < hashResult.size) {
                    hashResult.index += 1;
                    //console.log(hashResult.length + " - " + hashResult.index);  //测试
                    websocket.send(hashResult.length + " : " + hashResult.index);
                    ...
                    if (error === false) {
                      if (hashes) {
                        ...
                        websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : hashes出错");
                      }
                      await scheduler.wait(1000);
                      if (hashResult.offset < hashResult.size) {
                        hashResult.limit += 1;
                        if (hashResult.limit === 30) {
                          hashResult.limit = 0;
                          //await endCache();
                  ...
                  websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : nextHash超出apiCount限制");
                  await updateChat();
                  //await endCache();
                  await insertCache();  //测试
                  await close();
                ...
                stop = 3;
                //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : nextHash超时");
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : nextHash超时");
                await updateChat();
                //await endCache();
                await insertCache();  //测试
                await close();
          ...
          async function insertMedia() {
            ...
            try {
              ...
              if (mediaInfo.success === true) {
                ...
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入media数据成功");
                return mediaInfo.meta.last_row_id;
              } else {
                //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入media数据失败");
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入media数据失败");
                return 0;
              }
            } catch (e) {
              //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : insertMedia出错 : " + e);;
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : insertMedia出错 : " + e);
              await scheduler.wait(10000);
              await insertMedia();
            }
          }
          ...
          async function insertMediaIndex(id) {
            ...
            try {
              ...
              if (indexInfo.success === true) {
                ...
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] 插入mediaIndex数据成功");
              } else {
                //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入mediaIndex数据失败");
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] 插入mediaIndex数据失败");
              }
            } catch (e) {
              //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : insertMediaIndex出错 : " + e);
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] insertMediaIndex出错 : " + e);
              await scheduler.wait(10000);
          ...
          //     websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : selectPhotoIndex出错 : " + e);
          //     await scheduler.wait(10000);
          ...
          async function selectPhoto() {
            ...
            try {
              ...
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : selectPhoto出错 : " + e);
              await scheduler.wait(10000);
              await selectPhoto();
            }
          }
          ...
          async function insertPhoto() {
            ...
            try {
              ...
              if (photoInfo.success === true) {
                ...
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : (" + hashResult.photoLength +"/" + (hashResult.photoIndex + 1) + ") 插入photo数据成功");
                return photoInfo.meta.last_row_id;
              } else {
                //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入photo数据失败");
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : (" + hashResult.photoLength +"/" + (hashResult.photoIndex + 1) + ") 插入photo数据失败");
                return 0;
              }
            } catch (e) {
              ...
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : (" + hashResult.photoLength +"/" + (hashResult.photoIndex + 1) + ") insertPhoto出错 : " + e);
              await scheduler.wait(10000);
              await insertPhoto();
            }
          }
          ...
          async function insertPhotoIndex(id) {
            ...
            try {
              ...
              if (photoInfo.success === true) {
                ...
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] 插入photoIndex数据成功");
              } else {
                //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入photoIndex数据失败");
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] 插入photoIndex数据失败");
              }
            } catch (e) {
              //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : insertPhotoIndex出错 : " + e);
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] insertPhotoIndex出错 : " + e);
              await scheduler.wait(10000);
          ...
          async function endMessage(type) {
            if (stop === 0) {
              ...
              if (hashResult.hash.length === hashResult.count) {
                ...
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : count不一至 : " + hashResult.count + " - " + hashResult.hash.length);
                return 0;
              }
              //offsetId += 1;
              //messageIndex += 1;
          ...
          async function selectMessage() {
            ...
            try {
              ...
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : selectMessage出错 : " + e);
              await scheduler.wait(10000);
              await selectMessage();
            }
          }
          ...
          async function insertMessage() {
            ...
            try {
              ...
              if (messageInfo.success === true) {
                ...
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入message数据成功");
              } else {
                //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入message数据失败");
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 插入message数据失败");
              }
            } catch (e) {
              //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : insertMessage出错 : " + e);;
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : insertMessage出错 : " + e);
              await scheduler.wait(10000);
              await insertMessage();
            }
          }
          ...
          async function endInsert() {
            ...
            if (parseInt(messageCount) === 0) {
              ...
              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : message已在数据库中");
            }
          }
              ...
              if (stop === 0) {
                if (Date.now() - timeStart < 300000 || (hashResult.index > 0 && hashResult.length - hashResult.index < 50)) {
                  if (apiCount < 900) {
                    ...
                    websocket.send("(" + currentStep + ")endStep超出apiCount限制");
                    await updateChat();
                    //await endCache();
                    //await insertCache();  //测试
                    await close();
                  ...
                  stop = 3;
                  //console.log("(" + currentStep + ")endStep超时");
                  websocket.send("(" + currentStep + ")endStep超时");
                  await updateChat();
                  //await endCache();
                  //await insertCache();  //测试
                  await close();
            ...
            if (stop === 0) {
              ...
              if (Date.now() - timeStart < 300000 || (hashResult.index > 0 && hashResult.length - hashResult.index < 50)) {
                if (apiCount < 900) {
                        ...
                        if (messageArray[messageIndex].media.document) {
                          ...
                          if (hashResult.id && hashResult.accessHash) {
                            ...
                            if (mediaIndexResult) {
                              ...
                              if (mediaIndexCount === 0) {
                                ...
                                if (mediaResult) {
                                  ...
                                  if (mediaCount === 0) {
                                    ...
                                    websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 准备查询视频的hash");
                                    ...
                                    websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 视频已入过库了");
                                  ...
                                  websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 视频的mediaResult错误");
                                  await end();
                                }
                              } else {
                                //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 视频已入过索引库了");
                                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 视频已入过索引库了");
                              ...
                              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 视频的mediaIndexResult错误");
                              await end();
                            }
                          } else {
                            //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 视频的id或accessHash错误");
                            websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 视频的id或accessHash错误");
                            await end();
                          }
                          ...
                          if (hashResult.id && hashResult.accessHash) {
                            ...
                            if (hashResult.photoLength && hashResult.photoLength > 0) {
                              ...
                              for (hashResult.photoIndex; hashResult.photoIndex < hashResult.photoLength; hashResult.photoIndex++) {
                                ...
                                if (photoResult) {
                                  ...
                                  if (photoCount === 0) {
                                    ...
                                    websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : (" + hashResult.photoLength +"/" + (hashResult.photoIndex + 1) + ") 准备查询图片"+ hashResult.type + "的hash");
                                    hashResult.sender = await client.getSender(hashResult.dcId);
                                    hashResult.count = Math.ceil(hashResult.size / 131072);
                                    hashResult.length = Math.ceil(hashResult.size / (131072 * 8));
                                    if (hashResult.length > 0) {
                                    ...
                                    websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : (" + hashResult.photoLength +"/" + (hashResult.photoIndex + 1) + ") 图片"+ hashResult.type + "已入过库了");
                                  ...
                                  websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 图片的photoResult错误");
                                }
                              ...
                              websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 图片的info错误");
                              await end();
                            }
                          } else {
                            //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 图片的id或accessHash错误");
                            websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 图片的id或accessHash错误");
                            await end();
                          }
                        // } else {
                        //   console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 未知的媒体");
                        //   websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 未知的媒体");
                        }
                      } else {
                        //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 消息不包含媒体");
                        websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 消息不包含媒体");
                        await end();
                      }
                    } else {
                      //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 错误的消息");
                      websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : 错误的消息");
                      await end();
                    }
                  } else {
                    //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : messageIndex错误");
                    websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : messageIndex错误");
                    messageIndex = 0;
                    messageLength = 0;
                    messageArray = [];
                    await next();
                  ...
                  websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : nextMessage超出apiCount限制");
                  await updateChat();
                  //await endCache();
                  //await insertCache();  //测试
                  await close();
                ...
                stop = 3;
                //console.log("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : nextMessage超时");
                websocket.send("(" + currentStep + ")[" + messageLength +"/" + hashResult.messageIndex + "] " + offsetId + " : nextMessage超时");
                await updateChat();
                //await endCache();
                //await insertCache();  //测试
                await close();
              ...
              if (Date.now() - timeStart < 300000 || (hashResult.index > 0 && hashResult.length - hashResult.index < 50)) {
                if (apiCount < 900) {
                  ...
                  if (messageLength > 0) {
                    ...
                    websocket.send("(" + currentStep + ")messageLength : " + messageLength);
                    if (stop === 0) {
                      await checkCache();
                      await nextMessage();
                    } else if (stop === 1) {
                    ...
                    websocket.send("(" + currentStep + ")messageCount : " + messageCount);
                    offsetId += limit;
                    //messageIndex = 0;
                    // hashResult = {
                    //   "hash": [],
                    ...
                    if (fromPeer) {
                      ...
                      //websocket.send("(" + currentStep + ")全部chat采集完毕");
                      websocket.send("over");
                      await close();
                    }
                  }
                  ...
                  websocket.send("(" + currentStep + ")nextStep超出apiCount限制");
                  await updateChat();
                  //await endCache();
                  //await insertCache();  //测试
                  await close();
                ...
                stop = 3;
                //console.log("(" + currentStep + ")nextStep超时");
                websocket.send("(" + currentStep + ")nextStep超时");
                await updateChat();
                //await endCache();
                //await insertCache();  //测试
                await close();
              ...
              if (filter === Api.InputMessagesFilterVideo || filter === Api.InputMessagesFilterPhotoVideo) {
                if (messageArray[messageIndex].media) {
                  if (messageArray[messageIndex].media.document.id.toString() === cacheHashResult.id || messageArray[messageIndex].media.document.accessHash.toString() === cacheHashResult.accessHash) {
                    ...
                    websocket.send("从(" + hashResult.length + " | " + hashResult.index + ")处继续");
                  }
                }
              }
            }
          ...
          if (data === "start") {
            ...
            await getConfig();
            //await switchType();
            //websocket.send(fromPeer);  //测试
            // if (fromPeer) {
            //   if (hashResult.length && hashResult.length > 0) {
            //     websocket.send("继续获取下一个hash");
            //     if (stop === 0) {
            //       await nextHash();
            //     } else if (stop === 1) {
            //       websocket.send("pause");
            ...
            if (fromPeer) {
              ...
              if (stop === 0) {
                ...
                messageLength = messageArray.length;
                if (messageLength > 0) {
                  websocket.send("messageLength : " + messageLength);
                  if (stop === 0) {
                    await checkCache();
                    await nextMessage();
                  } else if (stop === 1) {
            ...
            if (cacheResult === true) {
              ...
              websocket.send("删除cache成功");
            } else {
              //console.log("删除cache失败");
              websocket.send("删除cache失败");
            }
          } else if (data === "count") {
            const mediaResult = await countMedia();
            if (mediaResult >= 0) {
              //console.log(mediaResult);
              websocket.send(mediaResult);
            } else {
              //console.log("获取media失败");
              websocket.send("获取media失败");
            }
          } else if (data === "chat") {
            if (!client) {
              await open();
              ...
              if (channelId && accessHash) {
                ...
                if (chatResult && chatResult["COUNT(id)"] === 0) {
                  ...
                  if (chatInfo.success === true) {
                    ...
                    websocket.send("插入chat数据成功");
                  } else {
                    //console.log("插入chat数据失败");
                    websocket.send("插入chat数据失败");
                  }
                // } else {
                //   //console.log("chat已在数据库中");
                //   websocket.send("chat已在数据库中");
                }
              } else {
                //console.log("chat的channelId或accessHash错误");
                websocket.send("chat的channelId或accessHash错误");
              }
            }
            //console.log("新插入了" + count + "条数据");
            websocket.send("新插入了" + count + "条数据");
            await close();
          } else if (data === "backup") {
            const signed_url = await exportDB();
            if (signed_url) {
              websocket.send(signed_url);
            } else {
              websocket.send("获取signed_url失败");
            }
          } else {
            websocket.send("未知消息");
          }
