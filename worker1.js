...
export class WebSocketServer extends DurableObject {
  ...
  async insertMessage() {
    try {
      const messageInfo = await this.env.MAINDB.prepare("INSERT INTO `MESSAGE` (id, dbIndex, category, txt, ids, status) VALUES (?, ?, ?, ?, ?, ?);").bind(this.hashResult.messageId, 6, this.hashResult.category, this.hashResult.txt, JSON.stringify(this.hashResult.ids), this.hashResult.status).run();
