...
export class WebSocketServer extends DurableObject {
  ...
  //     while (!this.fromPeer) {
  //       const chatResult = await env.MAINDB.prepare("SELECT * FROM `CHAT` WHERE `Cindex` >= ? AND `exist` = 1 LIMIT 1;").bind(this.chatId).first();
  //       //console.log("chatResult : " + chatResult"]);  //测试
  //       if (chatResult) {
  ...
  // async updateConfig() {
  //   try {
  //     const chatInfo = await env.MAINDB.prepare("UPDATE `CONFIG` SET `chatId` = ? WHERE `name` = 'collect';").bind(this.chatId).run();
  //     //console.log(chatInfo);  //测试
  //     if (chatInfo.success === true) {
  ...
  //   try {
  //     const mediaResult = await env.MAINDB.prepare("SELECT `Vindex`, COUNT(id) FROM `MEDIAINDEX` WHERE `id` = ? AND `accessHash` = ? LIMIT 1;").bind(this.hashResult.id, this.hashResult.accessHash).first();
  ...
  // async selectMedia() {
  //   try {
  //     const mediaResult = await env.MEDIADB.prepare("SELECT `Vindex`, COUNT(id) FROM `MEDIA` WHERE `id` = ? AND `accessHash` = ? LIMIT 1;").bind(this.hashResult.id, this.hashResult.accessHash).first();
  //     //console.log(mediaResult);  //测试
  //     return mediaResult;
  //   } catch (e) {
  ...
  //       //const cacheInfo = await env.MAINDB.prepare("INSERT INTO `CACHE` (chatId, id, accessHash, hash) VALUES (?, ?, ?, ?);").bind(this.chatId, this.hashResult.id, this.hashResult.accessHash, JSON.stringify(this.hashResult)).run();
  ...
  // //     delete this.hashResult.location;
  // //     delete this.hashResult.sender;
  // //     const cacheInfo = await env.MAINDB.prepare("UPDATE `CACHE` SET `hash` = ? WHERE `id` = ? AND `accessHash` = ?;").bind(JSON.stringify(this.hashResult), this.hashResult.id, this.hashResult.accessHash).run();
  // //     //console.log(cacheInfo);  //测试
  // //     if (cacheInfo.success === true) {
  ...
  // // async selectCache() {
  // //   try {
  // //     const cacheResult = await env.MAINDB.prepare("SELECT COUNT(id) FROM `CACHE` WHERE `id` = ? AND `accessHash` = ? LIMIT 1;").bind(this.hashResult.id, this.hashResult.accessHash).first();
  ...
  //   try {
  //     const indexInfo = await env.MAINDB.prepare("INSERT INTO `MEDIAINDEX` (Vindex, id, accessHash) VALUES (?, ?, ?);").bind(id, this.hashResult.id, this.hashResult.accessHash).run();
  ...
  // //   try {
  // //     const photoResult = await env.MAINDB.prepare("SELECT `Pindex`, COUNT(id) FROM `PHOTOINDEX` WHERE `id` = ? AND `accessHash` = ? AND `sizeType` = ? LIMIT 1;").bind(this.hashResult.id, this.hashResult.accessHash, this.hashResult.type).first();
  ...
  // async selectPhoto() {
  //   try {
  //     const photoResult = await env.PHOTODB.prepare("SELECT `Pindex`, COUNT(id) FROM `PHOTO` WHERE `id` = ? AND `accessHash` = ? AND `sizeType` = ? LIMIT 1;").bind(this.hashResult.id, this.hashResult.accessHash, this.hashResult.type).first();
  //     //console.log(photoResult);  //测试
  //     return photoResult;
  //   } catch (e) {
  ...
  // async insertPhoto() {
  //   try {
  //     const photoInfo = await env.PHOTODB.prepare("INSERT INTO `PHOTO` (id, accessHash, dcId, sizeType, size, hash) VALUES (?, ?, ?, ?, ?, ?);").bind(this.hashResult.id, this.hashResult.accessHash, this.hashResult.dcId, this.hashResult.type, this.hashResult.size, JSON.stringify(this.hashResult.hash)).run();
  ...
  //   try {
  //     const photoInfo = await env.MAINDB.prepare("INSERT INTO `PHOTOINDEX` (Pindex, id, accessHash) VALUES (?, ?, ?);").bind(id, this.hashResult.id, this.hashResult.accessHash).run();
  //     //console.log(photoInfo);  //测试
  //     if (photoInfo.success === true) {
  ...
  // async selectMessage() {
  //   try {
  //     const messageResult = await env.MAINDB.prepare("SELECT COUNT(id) FROM `MESSAGE` WHERE `id` = ? LIMIT 1;").bind(this.hashResult.messageId).first();
  ...
  // async insertMessage() {
  //   try {
  //     const messageInfo = await env.MAINDB.prepare("INSERT INTO `MESSAGE` (id, dbIndex, category, txt, ids, status) VALUES (?, ?, ?, ?, ?, ?);").bind(this.hashResult.messageId, 5, this.hashResult.category, this.hashResult.txt, JSON.stringify(this.hashResult.ids), this.hashResult.status).run();
