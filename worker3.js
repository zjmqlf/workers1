      ...
      async function handleSession(websocket) {
        ...
        websocket.addEventListener("message", async ({ data }) => {
          ...
          async function insertCache() {
            if (hashResult && hashResult.category === 2 && hashResult.hash && hashResult.hash.length && hashResult.hash.length > 0) {
              ...
              try {
                ...
                if (cacheInfo.success === true) {
                  ...
                  websocket.send(JSON.stringify({
                    "operate": "insertCache",
                    "step": currentStep,
                    "message": "插入cache("+ hashResult.length + " | " + hashResult.index + ")数据成功",
                    "date": Date.now(),
                  }));
                } else {
                  ...
                  websocket.send(JSON.stringify({
                    "operate": "insertCache",
                    "step": currentStep,
                    "message": "插入cache("+ hashResult.length + " | " + hashResult.index + ")数据失败",
                    "error": true,
                    "date": Date.now(),
                  }));
                }
                ...
                websocket.send(JSON.stringify({
                  "operate": "insertCache",
                  "step": currentStep,
                  "message": "("+ hashResult.length + " | " + hashResult.index + ")出错 : " + e,
                  "error": true,
                  "date": Date.now(),
                }));
                await scheduler.wait(10000);
              ...
              websocket.send(JSON.stringify({
                "operate": "insertCache",
                "step": currentStep,
                "message": "cache("+ hashResult.length + " | " + hashResult.index + ")数据错误",
                "error": true,
                "date": Date.now(),
              }));
            }
            ...
            try {
              ...
              if (photoInfo.success === true) {
                ...
                websocket.send(JSON.stringify({
                  "offsetId": offsetId,
                  "operate": "insertPhoto",
                  "photoIndex": ++hashResult.photoIndex,
                  "status": "success",
                  "date": Date.now(),
                }));
            ...
            if (stop === 0) {
              ...
              if (hashResult.hash.length === hashResult.count) {
                ...
                websocket.send(JSON.stringify({
                  "offsetId": offsetId,
                  "operate": "endMessage",
                  "message": "count不一至 : " + hashResult.count + " - " + hashResult.hash.length,
                  "error": true,
                  "status": "error",
                  "date": Date.now(),
                }));
          ...
          async function insertMessage() {
            apiCount += 1;
            try {
              const messageInfo = await env.MAINDB.prepare("INSERT INTO `MESSAGE` (id, dbIndex, category, txt, ids, status) VALUES (?, ?, ?, ?, ?, ?);").bind(hashResult.messageId, 5, hashResult.category, hashResult.txt, JSON.stringify(hashResult.ids), hashResult.status).run();
              ...
              if (Date.now() - timeStart < 300000 || (hashResult.index > 0 && hashResult.length - hashResult.index < 50)) {
                if (apiCount < 900) {
                  if (messageIndex >= 0 && messageIndex < limit) {
                    ...
                    if (messageArray[messageIndex]) {
                      ...
                      websocket.send(JSON.stringify({
                        ...
                        "messageIndex": hashResult.messageIndex,
                        "messageId": hashResult.messageId,
                        "status": "add",
                        "time": time,
                        "date": time,
                      }));
                                ...
                                if (mediaResult) {
                                  ...
                                  if (mediaCount === 0) {
                                    ...
                                    websocket.send(JSON.stringify({
                                      "offsetId": offsetId,
                                      "operate": "nextMessage",
                                      "category": hashResult.category,
                                      "dcId": hashResult.dcId,
                                      "size": hashResult.size,
                                      "type": hashResult.mimeType,
                          ...
                          if (hashResult.id && hashResult.accessHash) {
                            ...
                            if (hashResult.photoLength && hashResult.photoLength > 0) {
                              ...
                              websocket.send(JSON.stringify({
                                ...
                                "operate": "nextMessage",
                                "category": hashResult.category,
                                "photoLength": hashResult.photoLength,
                                "status": "update",
                                "date": Date.now(),
                              }));
                  ...
                  websocket.send(JSON.stringify({
                    "offsetId": offsetId,
                    "operate": "nextMessage",
                    "message": "nextMessage超出apiCount限制",
                    "error": true,
                    "status": "limit",
                    "date": Date.now(),
                  }));
              ...
              if (filter === Api.InputMessagesFilterVideo || filter === Api.InputMessagesFilterPhotoVideo) {
                if (messageArray[messageIndex].media) {
                  if (messageArray[messageIndex].media.document.id.toString() === cacheHashResult.id || messageArray[messageIndex].media.document.accessHash.toString() === cacheHashResult.accessHash) {
                    ...
                    websocket.send(JSON.stringify({
                      "operate": "checkCache",
                      "message": "从(" + hashResult.length + " | " + hashResult.index + ")处继续",
                      "date": Date.now(),
                    }));
                  }
                }
